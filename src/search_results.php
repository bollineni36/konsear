<?php 
		...
		$dayend = date('Y-m-d 00:00:00');
			$query = sprintf("SELECT * FROM %s%s WHERE OfferActiveFlag = 'Active' AND (OfferName LIKE '%s' OR OfferDesc LIKE '%s') AND OfferStartDate <= '$daystart' AND OfferEndDate >= '$dayend' ORDER BY PublisherOfferId DESC", DB_PREFIX, PUBLISHEROFFERS_TABLE, $search, $search);
