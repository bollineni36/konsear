		...
		if(isset($_POST['submit']))
		{
			...
			if($message == ""){
				...
				if(isset($row) && (decrypt($row['SubscriberPassword']) == $password)) {
						...
						$_SESSION['subid'] = $row['SubscriberId'];
						
						$_SESSION['sname'] = $row['SubscriberName'];
						
						$_SESSION['emailid'] = $row['SubscriberEmailId'];
						
						$_SESSION['zipcode'] = $row['SubscriberZipcode'];
						
						unset($_SESSION['pubid']);
						
						redirectPage('lookup_offers.php');
	    ...
	    <div class="medium-6 columns">
			 <form name="login" id="login" action="<?php echo $_SERVER['PHP_SELF']?>" method="POST" class="size">
				<div class="medium-12 columns bottomspace">
					...
					<h3 class="lightgrey">Subscriber Login Form</h3>
					<br/>
					...
					<div class="medium-7 columne">
						<p class="textcenter"><a href="sub_forgot_password.php">Forgot Password</a></p>
						<p class="textcenter"><a href="sub_register.php">New Subscriber?</a></p>
					</div>
				</div>
			</form>
		</div> 
