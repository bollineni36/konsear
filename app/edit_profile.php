	...
	if(isset($_POST['submit']))
	{
		...
		if($message == "")
		{
			$image = "";
				...
				$query = sprintf("UPDATE %s%s SET PublisherName = '%s', PublisherAddress1 = '%s',PublisherCity = '%s', PublisherState = '%s',PublisherZipcode = '%s', PublisherCountry = '%s',PublisherContactName = '%s' WHERE PublisherId = '%s'", DB_PREFIX, PUBLISHER_TABLE, $bname, $address, $city, $state, $zip, $country, $cname, $pubid);
				...
				//	$upload_exts = end(explode(".", $_FILES["file"]["name"]));
					...
					&& ($_FILES["file"]["size"] < 5000000))
					{
						...
						else
						{
							...
							$extension = ".".$file_ext;
							$image = $pubid.$extension;
							$path = "uploads/publisher/" . $image;
							
							move_uploaded_file($_FILES["file"]["tmp_name"], "uploads/publisher/" . $image);
							
							$query = sprintf("UPDATE %s%s SET PublisherImage = '%s' WHERE PublisherId = '%s'", DB_PREFIX, PUBLISHER_TABLE, $path, $pubid);
	    ...
	    <div class="medium-6 columns">
			 <form name="register" id="register" action="<?php echo $_SERVER['PHP_SELF']?>" method="POST" class="size" enctype="multipart/form-data" >
				<div class="medium-12 columns bottomspace">
					...
					<h3 class="lightgrey">Edit Profile</h3>
					<?php if($path != "") { ?>
						<p class="textcenter"><img src="<?php echo BASE_URL.$path?>"></p>
					<?php } ?>	 
