<?php 
	...
	require_once("../includes/validations.php");
		...
		if(isset($_POST['back']))
		{
			redirectPage('admin/manage_subscribers.php');
		}
		if(isset($_POST['submit']))
		{
			...
			$query = sprintf("UPDATE %s%s SET SubscriberActiveFlag = '%s' WHERE SubscriberId = '%s'", DB_PREFIX, SUBSCRIBERS_TABLE, $status, $sid);
	    ...
	    <div class="medium-6 columns">
			 <form name="editpublisher" id="editpublisher" action="<?php echo $_SERVER['PHP_SELF']?>" method="POST" class="size" enctype="multipart/form-data" >
				<div class="medium-12 columns bottomspace">
					<?if($path != "") { ?><div class="textcenter"><img src="<?php echo '../android/'.$path?>"></div><?php } ?>
					<?php if($message != "") echo "<div class='alert-box success radius'>".$message."</div>"; ?>
					<h3 class="lightgrey">Edit Subscriber</h3>
					<input type="hidden" name="path" value="<?php echo $path?>">
					<div class="medium-5 columns">
						<label class="inline">Subscriber Name</label>
					</div>
