<?php 
		...
		if($message == "")
		{
			$image = "";
				...
				if($stat == "Inactive" && $status == "Active")
				{
					...
					if($count == 1 && $plan == "1")
					{
						$_SESSION['errormsg'] = "Starter Pack members can have only one active account";
						redirectPage('accounts.php');
					}
					if($count == 2 && $plan = "2")
					{
						$_SESSION['errormsg'] = "Silver Pack members can have only 2 active accounts";
						redirectPage('accounts.php');
					}
				}
				
			
				$query = sprintf("UPDATE %s%s SET AccountName = '%s', AccountAddress1 = '%s',AccountCity = '%s', AccountState = '%s',AccountZipcode = '%s', AccountCountry = '%s',AccountContactName = '%s', AccountWebsite = '%s', AccountEmailId = '%s', AccountPhone='%s', InterestHeaderId = '%s', PublisherActiveFlag = '%s' WHERE AccountId = '%s'", DB_PREFIX, ACCOUNT_TABLE, $bname, $address, $city, $state, $zip, $country, $cname, $website, $email, $phone, $category, $status, $acid);
	...
	else
	{
		...
		$query = sprintf("SELECT * FROM %s%s WHERE AccountId = '%s' AND PublisherId='%s'", DB_PREFIX, ACCOUNT_TABLE, $acid, $pubid);
						
		$row = $db->getRow($query); 
		
		if(isset($row))
		{
			$bname = $row['AccountName'];
			$address = $row['AccountAddress1'];
			$city = $row['AccountCity'];
			$state = $row['AccountState'];
			$zip = $row['AccountZipcode'];
			$country = $row['AccountCountry'];
			$cname = $row['AccountContactName'];
			$website = $row['AccountWebsite'];
			$email = $row['AccountEmailId'];
			$phone = $row['AccountPhone'];
			$status = $row['PublisherActiveFlag'];
			$category = $row['InterestHeaderId'];
...
<html class="no-js" lang="en" data-useragent="Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0)">
  ...
  <!--<div class="row">
	    ...
	    <div class="medium-6 columns">
			 <form name="register" id="register" action="<?php echo $_SERVER['PHP_SELF']?>" method="POST" class="size" enctype="multipart/form-data" >
				<div class="medium-12 columns bottomspace">
					...
					<h3 class="lightgrey">Edit Account</h3>
					<?php if($path != "") { ?>
    ...
    <option value="ZW" <?PHP if($country=='ZW') echo $sel ?>>Zimbabwe</option>
					...
					<div class="medium-7 columns">
						<input type="text" name="website" id="website" value="<?php if(isset($website)) echo stripslashes($website); ?>"><span class="vrequire">*</span>
					</div>	
					<div class="medium-5 columns">
