<?php 
	...
	//	$terms   	= isset($_POST['terms']) ? $_POST['terms'] : NULL;
		...
		{
			$message .= "Publisher Age Must Be 21+";
		}
		...
		if($message == "")
		{
			...
			if($email1 == '')
			{
				...
				{	
					...
					$success = "No";
					$enddate = "";
					$message = "Thank You for signing up..! Please check your Email for login details";	
				}
				elseif($plan ==2)
				{	
				...
				else
				{
					...
					$message = "Thank You for signing up..! Please pay the amount via Paypal and check your Email for login details";	
				...
				//$enddate = date('Y-m-d H:i:s', strtotime("+1 day"));
				$sql = sprintf("INSERT INTO %s%s (PublisherName, PublisherAddress1, PublisherCity, PublisherState, PublisherZipcode, PublisherCountry, PublisherContactName, PublisherWebsite, PublisherTermAcceptanceFlag, PublisherEmailId, PublisherPassword, PublisherPlanId, PublisherStartDate, PublisherEndDate, PublisherActiveFlag) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','Yes','%s','%s','%s',NOW(), '%s', '%s')", DB_PREFIX, PUBLISHER_TABLE, $bname, $address, $city, $state, $zip, $country, $cname, $website, $email, $password, $plan, $enddate, $status);
				...
				//$sql1 = sprintf("INSERT INTO %s%s (PublisherId, AccountName	, AccountAddress1, AccountCity, AccountState, AccountZipcode, AccountCountry, AccountContactName, AccountWebsite, AccountEmailId, InterestHeaderId, PublisherActiveFlag) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')", DB_PREFIX, ACCOUNT_TABLE, $pid, $bname, $address, $city, $state, $zip, $country, $cname, $website, $email, $category, $status);
				
				$sql1 = sprintf("INSERT INTO %s%s (PublisherId, AccountName	, AccountAddress1, AccountCity, AccountState, AccountZipcode, AccountCountry, AccountContactName, AccountWebsite, AccountEmailId, InterestHeaderId, PublisherActiveFlag) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','Active')", DB_PREFIX, ACCOUNT_TABLE, $pid, $bname, $address, $city, $state, $zip, $country, $cname, $website, $email, $category);
				$db->insertRow($sql1);
			...
			//	$upload_exts = end(explode(".", $_FILES["file"]["name"]));
				...
				&& ($_FILES["file"]["size"] < 5000000))
				{
					...
					else
					{
						...
						$extension = ".".$file_ext;
						$image = $pid.$extension;
						$image2 = $sid.$extension;
						$path = "uploads/publisher/" . $image;
						$path2 = "uploads/publisher/account/" . $image2;
						
						move_uploaded_file($_FILES["file"]["tmp_name"], $path);
						copy($path, $path2);
						
						$query = sprintf("UPDATE %s%s SET PublisherImage = '%s' WHERE PublisherId = '%s'", DB_PREFIX, PUBLISHER_TABLE, $path, $pid);
							
						$row = $db->updateRow($query);
						
						$query1 = sprintf("UPDATE %s%s SET PublisherImage = '%s' WHERE AccountId = '%s'", DB_PREFIX, ACCOUNT_TABLE, $path2, $sid);
				...
				//echo $message;
					...
					/*Mail to Admin*/
					$subject1 = "New Publisher: ".$cname." (".$planname.")";
					$msg1 = 	"<table width='100%' align='center' style='background-color: #f0f0f0;'>
								<tr>
									<td>
										<br/><br/><br/>
										<table width='500px' align='center'>
											...
											<tr>
												<td>
													<p style='text-align: center;'>Hello Administrator,</p><br/><br/>
													<br/>
													A new Publisher has been joined in Konsear App, below are the details of the Publisher.
												</td>
											</tr>
											<tr>
												<td>
				...
				//echo $message;
					...
					$to		= ADMIN_EMAIL;
					//Mail to User
					$mailresult = sendEmail($from, $to, $subject1, $msg1);
				}
...
<html class="no-js" lang="en" data-useragent="Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0)">
  ...
  <!--<div class="row">
	    ...
	    <div class="medium-6 columns">
			 <form name="register" id="register" action="<?php echo $_SERVER['PHP_SELF']?>" method="POST" class="size" enctype="multipart/form-data" >
				<div class="medium-12 columns bottomspace">
					...
					<h3 class="lightgrey">Register Form</h3>
					<div class="medium-5 columns">
						<label class="inline">Choose Plan</label>
					</div>
					...
					<div class="medium-7 columns">
						<input type="text" name="bname" id="bname" value="<?php if(isset($bname)) echo $bname; ?>"><span class="vrequire">*</span>
					</div>
					<div class="medium-5 columns">
						<label class="inline">Email</label>
					</div>
					<div class="medium-7 columns">
						<input type="text" name="email" id="email" value="<?php if(isset($email)) echo $email; ?>"><span class="vrequire">*</span>
					</div>
					<div class="medium-5 columns">
						<label class="inline">Business Address</label>
					</div>
					<div class="medium-7 columns">
						<textarea name="address" id="address"><?php if(isset($address)) echo $address; ?></textarea><span class="vrequire">*</span>
					</div>
					<div class="medium-5 columns">
						<label class="inline">City</label>
					</div>
					<div class="medium-7 columns">
						<input type="text" name="city" id="city" value="<?php if(isset($city)) echo $city; ?>"><span class="vrequire">*</span>
					</div>
					<div class="medium-5 columns">
					...
					<div class="medium-7 columns">
						<input type="text" name="zip" id="zip" value="<?php if(isset($zip)) echo $zip; ?>"><span class="vrequire">*</span>
					</div>
					<div class="medium-5 columns">
    ...
    <option value="ZW" <?PHP if($country=='ZW') echo $sel ?>>Zimbabwe</option>
					...
					<div class="medium-7 columns">
						<input type="text" name="cname" id="cname" value="<?php if(isset($cname)) echo $cname; ?>"><span class="vrequire">*</span>
					</div>
					<div class="medium-5 columns">
						<label class="inline">Website</label>
					</div>
					<div class="medium-7 columns">
						<input type="text" name="website" id="website" value="<?php if(isset($website)) echo $website; ?>"><span class="vrequire">*</span>
					</div>
					<div class="medium-5 columns">
					...
					<div class="medium-7 columns">
						<input type="file" name="file" id="file" value=""><span class="vrequire">*</span>
					</div>
					...
					<div class="medium-1 columns">&nbsp;</div>
					<div class="medium-11 columns">
						<input type="checkbox" name="age" id="age" value="age"> <label>Age must be 21+</label>
					</div>
					<div class="row">
						<div class="medium-5 columns">
							&nbsp;
						...
						<div class="medium-7 columns">
							<div align="center" class="vbottomspace" style="width: 96%;">
					...
					<div class="medium-7 columns">
						<div align="center">
							<input type="submit" name="submit" value="Register" class="tiny button radius">
						</div>
					</div>
				</div>
			</form>
...
if(isset($success))
{
	...
	if($success == "Yes")
	{
		$_SESSION['paypalpage'] = "pay_register.php";
...
<input type="hidden" name="cancel_return" value="'.BASE_URL.'register.php">
